...
import cash.z.ecc.android.ext.toTxId
...
class HistoryViewModel : ViewModel() {
    ...
    val selectedTransaction = MutableStateFlow<TransactionOverview?>(null)
    ...
    suspend fun getAddress() = synchronizer.getUnifiedAddress()
    ...
    private suspend fun TransactionOverview?.toUiModel(latestHeight: Int? = null): UiModel =
        UiModel().apply {
            this@toUiModel.let { tx ->
                txId = tx?.rawId?.byteArray.toTxId()
                val toAddress = tx?.let {
                    synchronizer.getRecipients(it)
                        .firstOrNull { it is TransactionRecipient.Address } as TransactionRecipient.Address
                }
                isInbound = when {
                    !(toAddress?.addressValue.isNullOrEmpty()) -> false
                    tx != null && toAddress?.addressValue.isNullOrEmpty() && tx.netValue.value > 0L && tx.minedHeight.value > 0 -> true
                    else -> null
                }
                isMined =
                    tx?.minedHeight != null && tx.minedHeight.value > synchronizer.network.saplingActivationHeight.value
                topValue =
                    if (tx == null) "" else "\$${WalletZecFormmatter.toZecStringFull(tx.netValue)}"
                minedHeight = String.format("%,d", tx?.minedHeight ?: 0)
                val flags =
                    DateUtils.FORMAT_SHOW_DATE or DateUtils.FORMAT_SHOW_YEAR or DateUtils.FORMAT_ABBREV_MONTH
                timestamp =
                    if (tx == null || tx.blockTimeEpochSeconds <= 0) getString(R.string.transaction_timestamp_unavailable) else DateUtils.getRelativeDateTimeString(
                        ZcashWalletApp.instance,
                        tx.blockTimeEpochSeconds * 1000,
                ...
                // memo logic
                val txMemo = this@toUiModel?.let { synchronizer.getMemos(it).firstOrNull() }
                if (txMemo?.isNotEmpty() == true) {
                    memo = txMemo
                }
                ...
                tx?.let {
                    val isMined = it.blockTimeEpochSeconds != 0L
                    if (isMined) {
                        val hasLatestHeight =
                            latestHeight != null && latestHeight > synchronizer.network.saplingActivationHeight.value
                        if (it.minedHeight.value > 0 && hasLatestHeight) {
                            val confirmations = latestHeight!! - it.minedHeight.value + 1
                            confirmation =
                ...
                when (isInbound) {
                    true -> {
                        ...
                        bottomValue = "\$${WalletZecFormmatter.toZecStringFull(tx?.netValue)}"
                        iconRotation = 315f
                    ...
                    false -> {
                        ...
                        bottomValue =
                            "\$${WalletZecFormmatter.toZecStringFull(Zatoshi((tx?.netValue?.value ?: 0) + ZcashSdk.MINERS_FEE.value))}"
                        iconRotation = 135f
                        fee = "+ 0.00001 network fee"
                        source = getString(R.string.transaction_story_from_shielded)
                        address = toAddress?.addressValue
                    }
                    null -> {
    ...
    private fun isSufficientlyOld(tx: TransactionOverview): Boolean {
        val threshold = 75 * 1000 * 25 // approx 25 blocks
        val delta = System.currentTimeMillis() / 1000L - tx.blockTimeEpochSeconds
        return tx.minedHeight.value > synchronizer.network.saplingActivationHeight.value &&
            delta < threshold
    }
}
